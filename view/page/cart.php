        ...
        <?php if (count($carts) > 0) : ?>
                        ...
                        <?php foreach($carts as $cart) : ?>
                            <li class="collection-item avatar cart-item" data-id="<?=$cart['product_id']?>" data-url="<?=baseurl('cart/process')?>">
                                ...
                                <div class="row m-t-1 m-b-0">
                                    <div class="col s7 l4">
                                        <span class="price">R$ <?=number_format($cart['price'])?></span>
                                    </div>
                                    ...
                                    <div class="col s12 l6 right-align">
                                        <span class="item-total-price bold">R$ <?=number_format($cart['total_item_price'])?></span>
                                    </div>
                                </div>
                                <div class="secondary-content">
                                    <button type="button" class="delete-item red btn btn-small btn-floating waves-effect waves-light materialize-tooltip" data-position="right" data-tooltip="Excluir Produto"><i class="material-icons">clear</i></button>
                                </div>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                    <div class="right-align m-b-1">
                        <span class="cart-total-price total-price bold">R$ <?=number_format($total_price)?></span>
                    </div>
                </div>
                <div class="col s12 m6 l3 offset-l6 m-b-1">
                    <a href="<?=baseurl()?>" class="grey waves-effect waves-light btn btn-block">Adicionar mais produtos</a>
                </div>
                <div class="col s12 m6 l3 m-b-1">
                    <button type="button" class="btn-checkout btn btn-block wave-effect waves-light">Pagamento</button>
                </div>
            </div>
            ...
            <div class="col s12 form-checkout hide">
                <form class="col l8 s12 offset-l2" action="<?=baseurl('cart/checkout')?>" method="post">
                    <h5 class="grey-text">Insira Seus Dados</h5>
                    <br>
                    <div class="row">
                    ...
                    <div class="row">
                        <div class="col l4 s6">
                            <button type="submit" class="blue waves-effect waves-light btn btn-block">Avançar</button>
                        </div>
                        <div class="col l4 s6">
                            <button class="back-to-cart grey waves-effect waves-light btn btn-block">Voltar</button>
                        </div>
                    </div>
                </form>
            </div>
        <?php else : ?>
            <div class="cart-container">
                <div class="row center-align">
                    <div class="col s12">
                        <h1 class="title">Nenhum produto foi adicionado ao carrinho ainda...</h1>
                        <br><br>
                    </div>
                    <div class="col s12 l4 offset-l4">
                        <a href="<?=baseurl()?>" class="grey waves-effect waves-light btn btn-block">Adicionar Produto</a>
                    </div>
                </div>
            </div>
        <?php endif; ?>
